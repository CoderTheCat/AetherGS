#!/usr/bin/env python
"""
C1 v2.1 Room 场景测试 - 1000 迭代
多场景验证：验证 C1 v2.1 在 room 场景的效果
"""
...
def run_test(iterations=1000, output_path=None):
    ...
    if sys.platform == 'linux':
        lp.source_path = "/mnt/e/Code/LiteGS/data/360_v2/room"
    else:
        lp.source_path = r"e:\Code\LiteGS\data\360_v2\room"
    
    model_name = f"c1_v2.1_room_{iterations}iter"
    ...
    # 保存结果
    results = {
        "test_type": "C1_v2.1_MultiScene_Room",
        "version": "2.1",
        "config": {
            "scene": "Room",
    ...
    if output_path:
        ...
        result_file = output_dir / f"C1_v2.1_room_{iterations}iter_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(result_file, "w", encoding="utf-8") as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f"\n[3/3] 结果已保存到：{result_file}")
    
    print("\n" + "=" * 70)
    print("C1 v2.1 Room 场景测试完成")
    print("=" * 70)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="C1 v2.1 Room 场景测试")
